...
class Tree_Basic(Tree):
    ...
    def recursive_tree_build(self, type, c_list):
        ...
        for child in c_list:
            if child["inverted"]:
                composite.add_child(pt.decorators.Inverter(name="inverted_"+child["name"], child=self.behavior_dict[child["name"]]))
            elif child["type"] == "composite":
                composite.add_child(self.recursive_tree_build(child["name"], child["child_list"]))
            else:
                composite.add_child(self.behavior_dict[child["name"]])
        return composite
