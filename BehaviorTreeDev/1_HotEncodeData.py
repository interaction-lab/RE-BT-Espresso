...
def hot_encode_features(features_data, categorical_features):
    for categorical_feature in categorical_features:
        ...
        features_data[categorical_feature] = features_data[categorical_feature].astype(
            str)
...
def encode_label_column(label_column):
    ...
    label_column = np.array(
        label_encoder.transform(label_column.values.ravel()))
...
def run_hotencode(json_file_path, output_log_path):
    ...
    features_data = features_data.drop(
        columns=categorical_features + binary_features)
    ...
    # format hot encoded columns to binary features
    hot_encode_fmt = "%s," * len(hot_encoded_header + binary_features)
    # format numerical columns to doubles
    feature_data_fmt = "%1.3f," * len(features_data.columns)
    total_fmt = hot_encode_fmt + feature_data_fmt + "%i"  # for label

    final_header = ','.join(str(i) for i in (
        hot_encoded_header + binary_features + list(features_data.columns)))
