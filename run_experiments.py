...
def parse_args():
    ...
    return json_file_path, should_recolor, run_multiprocess, run_original_bt_espresso,run_my_method
...
def main():
    ...
    global experiments_folder, should_recolor, run_original_bt_espresso, run_my_method

    single_experiment_filename, should_recolor, run_multiprocess, run_original_bt_espresso, run_my_method = parse_args()
...
def run_experiment(experiment_file):
    global base_pipeline_config, should_recolor, run_original_bt_espresso, run_my_method
    ...
    bt_tree_filepath_list = run_pipeline.run_pipeline(
        pipeline_config_path, should_recolor, sim_data_output_path + "fmt.log", run_original_bt_espresso, run_my_method)
    simulated_tree_file = sim_data_output_path + sim_tree_name + ".dot"
    bt_tree_filepath_list.insert(0, simulated_tree_file)
    run_results.run_result_list(bt_tree_filepath_list[:-1], run_original_bt_espresso, run_my_method) # removes the tree with nothing in it / final prune
