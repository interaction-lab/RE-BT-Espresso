...
class timeseries_Header(Enum):
    ...
    # Covers Action Space: { Positive Physical Affect }
    kuriPhysicalAction = 22
...
def updateQueues(row, submitQueue, newExerciseQueue):
    submitQueue.append(
        str(row[timeseries_Header.ExerciseSubmissionResult.value]))
    newExerciseQueue.append(str(row[timeseries_Header.CurExercise.value]))

    if len(submitQueue) > 5:
        submitQueue.pop(0)
    if len(newExerciseQueue) > 5:
        newExerciseQueue.pop(0)

    return submitQueue, newExerciseQueue


def getSubmissionResult(submitList):
    if 'InCorrect' in submitList:
        return "InCorrect"
    elif 'Correct' in submitList:
        return "Correct"
    else:
...
def classifyDialogue(dialogue):
    if dialogue in Congratulations:
        return 2
    elif dialogue in Encouragement:
        return 3
    elif dialogue[:5] == "Goal:":
        return 1
    else:
        return 4
...
def makeModName(path):
    ...
    # pathStart + "clean_data/"
    newFolder = os.path.join(pathStart, "clean_data")
