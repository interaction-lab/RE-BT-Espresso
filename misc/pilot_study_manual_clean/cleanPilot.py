...
header_row = ["time", "visa", "currentTKC", "row[3]", "row[4]", "dialogue", "currentExerciseChange", "row[7]", "row[8]", "row[9]", "row[10]", "row[11]", "row[12]",
              "row[13]", "row[14]", "row[15]", "row[16]", "row[17]", "row[18]", "row[19]", "row[20]", "row[21]", "ppa", "row[23]", "row[24]", "KC", "tsla", "tw", "isThereMoreScaff"]
...
def makeModName(path):
    ...
    new_folder = os.fsdecode(os.path.join(root_path, folder_name))
    if not os.path.isdir(new_folder):
        os.makedirs(new_folder)
    new_file = os.fsdecode(os.path.join(
        new_folder, filename_root + modified_extension + filename_ext))
    return new_file
...
ap.add_argument("-p", "--path", required=True,
                help="Path to the folder with CSV(s)")
    ...
    if filename.endswith(".csv") or filename.endswith(".CSV"):
        ...
        with open(absolutePath) as csvfile, open(filenameEdited, mode='w') as csvfileMod:
            ...
            csv_writer = csv.writer(
                csvfileMod, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            ...
            for row in csv_reader:
                ...
                if time > (startingTime + warmUpBuffer):
                    if row[tools.timeseries_Header.robotKCLevel.value] != "":
                        currentTKC = float(
                            row[tools.timeseries_Header.robotKCLevel.value])
                    if row[tools.timeseries_Header.CurExercise.value] != "":
                        currentExerciseChange = True
                        currentExercise = int(
                            row[tools.timeseries_Header.CurExercise.value])
                        scaffoldLeft = tools.scaffSize.get(currentExercise)
                    # print("scaffoldLeft: {}".format(scaffoldLeft))
                    isThereMoreScaff = scaffoldLeft > 0
                    KC = tools.computeKC(row[tools.timeseries_Header.humanMoveZScore.value],
                                         row[tools.timeseries_Header.humanCurZScore.value])
                    tsla = time - lastActionTime

                    # DIALOGUE Label
                    if row[tools.timeseries_Header.kuriDialogue.value] != "":
                        dialogue = tools.getDialogue(
                            row[tools.timeseries_Header.kuriDialogue.value])
                    ...
                    if row[tools.timeseries_Header.kuriPhysicalAction.value] != "":
                        PPAdesc = str(
                            row[tools.timeseries_Header.kuriPhysicalAction.value])
                    ...
                    if canContinue:
                        if not(currentExerciseChange):
                            currentExerciseChange = ''
                        csvRow = [time, visa, currentTKC, row[3], row[4], dialogue, currentExerciseChange, row[7], row[8], row[9], row[10], row[11], row[12],
                                  row[13], row[14], row[15], row[16], row[17], row[18], row[19], row[20], row[21], ppa, row[23], row[24], KC, tsla, tw, isThereMoreScaff]
                        csv_writer.writerow(csvRow)
                else:
                    scaffoldLeft = tools.scaffSize.get(currentExercise)

modDataFolder = os.path.join(directory_in_str, folder_name)
