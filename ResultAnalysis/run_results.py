...
def parse_args():
    ...
    ap.add_argument("-p", "--pathtodot", required=True,
...
def generate_results(key, results_dict, graph):
    ...
    results_dict[key][unique_node_freq_key] = nh.get_freq_unique_node_dict(
        graph)
    if simulated_graph:
        results_dict[key][graph_sim_key] = nh.find_graph_sim(
